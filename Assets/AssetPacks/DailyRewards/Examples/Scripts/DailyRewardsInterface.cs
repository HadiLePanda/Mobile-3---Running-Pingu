...
namespace NiobiumStudios
{
    ...
    public class DailyRewardsInterface : MonoBehaviour
    {
        ...
        public TMP_Text textReward;                     // Reward Text to show an explanatory message to the player
        ...
        public TMP_Text textTimeDue;                    // Text showing how long until the next claim
        ...
        public void SnapToReward()
        {
            ...
            if (lastRewardIdx >= 0 && lastRewardIdx <= dailyRewardsUI.Count - 1)
            {
        ...
        private void CheckTimeDifference ()
        {
            if (!readyToClaim)
            {
                ...
                textTimeDue.text = string.Format("Next reward in {0}", formattedTs);
            }
        }
